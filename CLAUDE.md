...
## Notes
- `VISION.md` — what BotBeam *is*: the communication layer between people and their agents (glass made two-way, durable, addressable, agent-aware — never intelligent). Four capabilities (project work to the human world · human-in-the-loop without co-presence · intent inbox · chatmail artifacts) over the payload-exchange mechanism (send/retrieve, one-place fleet hub, liveness ladder). Read it before proposing features.
